...
import { Input, Select, Button, Card, Upload } from "antd";
import { PlusOutlined, MinusCircleOutlined, UploadOutlined } from "@ant-design/icons";

// ✅ Định nghĩa kiểu dữ liệu cho danh mục và màu sắc
interface Color {
  id: number;
  image: File | null;
}
      ...
      {categories.map((category) => (
        <Card key={category.id} className="mb-4">
          <div className="flex justify-between items-center mb-3">
            ...
            <Button danger icon={<MinusCircleOutlined />} onClick={() => removeCategory(category.id)}>
              Xóa
            </Button>
          </div>
          {category.visible && (
            <>
              <div className="mb-3 flex gap-3">
                <Input placeholder="Tên sản phẩm" className="w-1/3" />
                <Input placeholder="Giá thêm" className="w-1/3" />
                <Input placeholder="Giá giảm" className="w-1/3" />
              </div>
              <Button type="dashed" icon={<PlusOutlined />} onClick={() => addColor(category.id)}>
                Thêm Màu Sắc
              </Button>
              {category.colors.map((color) => (
                <Card key={color.id} className="mt-3">
                  <div className="flex justify-between items-center mb-3">
                    <Button danger icon={<MinusCircleOutlined />} onClick={() => removeColor(category.id, color.id)}>
                      Xóa
                    </Button>
                  </div>
                  <div className="mb-3 flex gap-3">
                    <Input placeholder="Tên màu" className="w-1/3" />
                    <Input placeholder="Giá thêm" className="w-1/3" />
                    <Input placeholder="Số lượng" className="w-1/3" />
                  </div>
                  <Upload>
